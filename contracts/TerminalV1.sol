...
/**
  ...
  @notice 
  This contract manages the Candybox ecosystem, serves as a payment terminal, and custodies all funds.
...
contract TerminalV1 is Operatable, ITerminalV1, ITerminal, ReentrancyGuard {
  ...
  function _takeFee(
    ...
    if (terminalDirectory.terminalOf(CandyboxProject(governance).projectId()) == this) {
      // Use the local pay call.
      _pay(CandyboxProject(governance).projectId(), feeAmount, _beneficiary, _memo, false);
    } else {
      // Use the external pay call of the governance contract.
      CandyboxProject(governance).pay{value: feeAmount}(_beneficiary, _memo, false);
    }
  }
}
